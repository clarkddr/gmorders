...
@elseif ($value <= $min)
    <span class="px-3 py-1 text-{{$size}} font-medium leading-5 text-white transition-colors duration-150 bg-{{ $below }}-700 dark:bg-{{ $below }}-600 border border-transparent rounded-md active:bg-{{ $below }}-600 hover:bg-{{ $below }}-800 dark:hover:bg-{{ $below }}-700 focus:outline-none focus:shadow-outline-{{ $below }}">
@else
    <span class="px-3 py-1 text-{{$size}} font-medium leading-5 text-gray-700 dark:text-gray-400 transition-colors duration-150 border border-transparent rounded-md focus:outline-none">
@endif
    {{$value . '%'}}
