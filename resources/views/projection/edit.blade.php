...
<x-layout>
	...
	<x-titlePage title="Proyección {{$projection['name']}}" subtitle="{{'Del '.Carbon::parse($projection['start'])->isoFormat('DD MMM YY').' al '.Carbon::parse($projection['end'])->isoFormat('DD MMM YY').''}}" >
        ...
        <div class="w-full overflow-x-auto">
            <table id="tableTotal" class="w-full whitespace-no-wrap rounded-lg shadow-xs">
                <thead>
                ...
                <tr class="px-4 py-3 mb-3 text-xs font-semibold tracking-wide text-gray-500 uppercase border-t dark:border-gray-700 bg-gray-50 sm:grid-cols-9 dark:text-gray-400 dark:bg-gray-800">
                    ...
                    <td class="px-4 py-3 font-bold text-xl text-right"> {{ $totals['sale'] }} </td>
                    <td class="px-4 py-3 font-bold text-xl text-right"> {{ $totals['purchase'] }} </td>
                    <td class="px-4 py-3 font-bold text-xl text-right"> {{ $totals['purchaseVsSale'] }}% </td>
